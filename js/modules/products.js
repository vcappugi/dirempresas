  ...
  try {
    ...
    if (productsList.length === 0) {
      ...
      productsList.forEach(prod => {
        ...
        row.innerHTML = `
          <td class="px-6 py-4 font-semibold text-slate-800 dark:text-white">${prod.id}</td>
          <td class="px-6 py-4 text-slate-650 dark:text-slate-255 font-medium">${escapeHtml(prod.nombre || '')}</td>
          <td class="px-6 py-4 text-slate-600 dark:text-slate-400 max-w-xs truncate">${escapeHtml(prod.descripcion || '-')}</td>
          <td class="px-6 py-4 text-slate-600 dark:text-slate-400 font-mono">${dateStr}</td>
          <td class="px-6 py-4 text-right space-x-1.5">
            ${window.hasPermission('view-products', 'escribir')
              ? `<button onclick="editProduct(${prod.id})" class="text-brand hover:text-brand-light text-xs font-semibold px-2.5 py-1.5 rounded-lg hover:bg-brand/10 transition-colors">Editar</button>
                 <button onclick="deleteProduct(${prod.id})" class="text-red-500 hover:text-red-650 text-xs font-semibold px-2.5 py-1.5 rounded-lg hover:bg-red-500/10 transition-colors">Eliminar</button>`
              : `<button onclick="editProduct(${prod.id})" class="text-brand hover:text-brand-light text-xs font-semibold px-2.5 py-1.5 rounded-lg hover:bg-brand/10 transition-colors">Ver</button>`
            }
          </td>
        `;
