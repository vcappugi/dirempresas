  ...
  try {
    ...
    if (detailTypesList.length === 0) {
      ...
      detailTypesList.forEach(item => {
        ...
        const btnEditar = `
          <button onclick="editDetailType(${item.id})" class="inline-flex items-center justify-center p-1.5 rounded-lg text-brand bg-brand/10 hover:bg-brand/20 dark:text-brand-light dark:bg-brand/15 dark:hover:bg-brand/25 transition-all duration-200 shadow-sm border border-brand/20 dark:border-brand/30" title="Editar Tipo de Detalle">
        ...
        const btnVer = `
          <button onclick="editDetailType(${item.id})" class="inline-flex items-center justify-center p-1.5 rounded-lg text-brand bg-brand/10 hover:bg-brand/20 dark:text-brand-light dark:bg-brand/15 dark:hover:bg-brand/25 transition-all duration-200 shadow-sm border border-brand/20 dark:border-brand/30" title="Ver Detalles">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        row.innerHTML = `
          ...
          <td class="px-4 py-3 text-slate-650 dark:text-slate-200 font-medium">${escapeHtml(item.tipo)}</td>
          <td class="px-4 py-3 text-slate-550 dark:text-slate-400 text-xs">${item.descripcion ? escapeHtml(item.descripcion) : '<span class="text-slate-400 italic">Sin descripción</span>'}</td>
          <td class="px-4 py-3 text-slate-550 dark:text-slate-400 font-mono text-xs">${item.orden !== null && item.orden !== undefined ? item.orden : '-'}</td>
        `;
